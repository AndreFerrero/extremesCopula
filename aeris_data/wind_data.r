...
wind_max_full <- df_daily$wind_max_daily

# 2. 4 seasonal vectors
wind_max_winter <- df_daily %>%
  filter(season == "winter") %>%
  pull(wind_max_daily)
wind_max_spring <- df_daily %>%
  filter(season == "spring") %>%
  pull(wind_max_daily)
wind_max_summer <- df_daily %>%
  filter(season == "summer") %>%
  pull(wind_max_daily)
wind_max_autumn <- df_daily %>%
  filter(season == "autumn") %>%
  pull(wind_max_daily)

xl <- 0
wind_pos_max_full <- df_daily %>%
  filter(wind_max_daily > xl) %>%
  pull(wind_max_daily)

# 4. 4 seasonal vectors (positive only)
wind_pos_max_winter <- df_daily %>%
  filter(season == "winter", wind_max_daily > xl) %>%
  pull(wind_max_daily)
wind_pos_max_spring <- df_daily %>%
  filter(season == "spring", wind_max_daily > xl) %>%
  pull(wind_max_daily)
wind_pos_max_summer <- df_daily %>%
  filter(season == "summer", wind_max_daily > xl) %>%
  pull(wind_max_daily)
wind_pos_max_autumn <- df_daily %>%
  filter(season == "autumn", wind_max_daily > xl) %>%
  pull(wind_max_daily)
...
save(
  ...
  file = "aeris_data/mtp_aereoport/wind_pos_hourly_data.RData"
)

save(
  wind_max_full,
  wind_max_winter, wind_max_spring, wind_max_summer, wind_max_autumn,
  file = "aeris_data/mtp_aereoport/wind_max_data.RData"
)

save(
  wind_pos_max_full,
  wind_pos_max_winter, wind_pos_max_spring,
	wind_pos_max_summer, wind_pos_max_autumn,
  file = "aeris_data/mtp_aereoport/wind_pos_max_data.RData"
)