...
p <- 0.85
plot(x)
abline(h = quantile(x, p), col = "red", lty = 2) # Threshold for GPD fit
sum(x > quantile(x, p)) # Number of exceedances
hist(x[x > quantile(x, p)], breaks = 20, main = "Exceedances over 85th Percentile", xlab = "Exceedance Value")
...
fit_egpd <- fit.extgp(x,
    model = 1, init = list(kappa = 1, sigma = sd(x), xi = 0.1),
    method = "mle"
)
...
u <- quantile(x, p)
fit_gpd <- fevd(x, threshold = u, type = "GP", method = "MLE")
