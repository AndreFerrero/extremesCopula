functions {
  ...
  // Vectorized LCDF
  vector egpd4_lcdf_vec(vector x, real mu, real kappa, real delta, real sigma, real xi) {
    int N = num_elements(x);
  ...
  // Vectorized LPDF
  real egpd4_lpdf_vec(vector x, real mu, real kappa, real delta, real sigma, real xi) {
    int N = num_elements(x);
    vector[N] log_G = gpd_lcdf_vec(x, mu, sigma, xi);
    vector[N] out;
    real a = 1.0 / delta;
    ...
    for (n in 1:N) {
      ...
      real log_gpd = gpd_lpdf(x[n] - mu | sigma, xi);
  ...
  real egpd4_quantile(real u, real mu, real kappa, real delta, real sigma, real xi, int I) {
    ...
    // 4. Invert GPD
    return mu + (sigma / xi) * (pow(1.0 - G, -xi) - 1.0);
  }
...
model {
  ...
  if (prior_check == 0) {
    target += egpd4_lpdf_vec(x, mu, kappa, delta, sigma, xi);
    vector[T] u = exp(egpd4_lcdf_vec(x, mu, kappa, delta, sigma, xi));
...
generated quantities {
  vector[T] x_rep;
  if (run_ppc == 1) {
    x_rep[1] = egpd4_quantile(uniform_rng(0.01, 0.99), mu, kappa, delta, sigma, xi, I);
    for (t in 2:T) {
      real u_prev = exp(egpd4_lcdf_vec(x_rep[(t-1):(t-1)], mu, kappa, delta, sigma, xi)[1]);
      ...
      x_rep[t] = egpd4_quantile((low_c + high_c) / 2.0, mu, kappa, delta, sigma, xi, I);
    }
  }
}
