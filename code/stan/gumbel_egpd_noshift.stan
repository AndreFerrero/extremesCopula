functions {
  ...
  real egpd_lpdf_vec(vector x,
    ...
    for (n in 1:N) {
      log_G[n] = gpd_lcdf(x[n] | sigma, xi);
      log_g[n] = gpd_lpdf(x[n] | sigma, xi);
    }
  ...
  real egpd_lcdf(real x,
    ...
    return kappa * gpd_lcdf(x | sigma, xi);
  }
  ...
  vector egpd_lcdf_vec(vector x,
    ...
    for (n in 1:N)
      out[n] = kappa * gpd_lcdf(x[n] | sigma, xi);
  ...
  real egpd_rng(real kappa,
                real sigma,
                real xi) {

    return egpd_quantile(uniform_rng(0, 1), kappa, sigma, xi);
  }
...
model {
  ...
  if (prior_check == 0) {
    ...
    target += egpd_lpdf_vec(x, kappa, sigma, xi);

    // =========================
    // Vectorized PIT
    // =========================

    vector[T] log_u = egpd_lcdf_vec(x, kappa, sigma, xi);
...
generated quantities {
  ...
  if (run_ppc == 1) {
    ...
    for (t in 2:T) {
      real v_prev = exp(egpd_lcdf(x_rep[t-1] | kappa, sigma, xi));
      ...
      x_rep[t] = egpd_quantile(u_next, kappa, sigma, xi);
    }
  }
}
