...
\begin{frame}{Classical Problems in Extremes Modelling}
    \begin{itemize}
        \item Classical methods require \textbf{preliminary}, and often \textbf{subjective}, choices before fitting the model
        \item Extremes are \textbf{scarse} and we need methods that avoid discarding useful information
        \item Ignoring \textbf{dependence} can make tail estimation difficult due to lower effective sample size
    \end{itemize}
\end{frame}

\begin{frame}{Overview}
    \begin{itemize}
        \item Review of classical and modern approaches to model \textbf{independent} extremes 
        \item Review of classical approaches to model \textbf{dependent} extremes
        \item Copula Markov Chains
    \end{itemize}
\end{frame}
...
% ==========================================
\section{Modelling Independent Extremes}
...
\begin{frame}{GPD: The Limit Distribution of Exceedances}
    ...
    \[
    P(X - u \le y \mid X > u) \xrightarrow{u \to x_+} H_{\xi, \sigma}(y) = \begin{cases}
...
\begin{frame}{Copula Markov Chains: Construction}
    ...
    A stationary (first-order) Markov chain $\{X_t\}$ can be constructed from a marginal distribution $F$ and a copula $C$ for the pair $(X_{t-1}, X_t)$.
        ...
        \begin{block}{Conditional Distribution}
            \[
            P(X_t \le x_t \mid X_{t-1}=x_{t-1})
            =
            \frac{\partial}{\partial u}C(u,v)
            \Big|_{u=F(x_{t-1}),\,v=F(x_t)}
            \]
        \end{block}
\end{frame}


\begin{frame}{Copula Markov Chains: Dependence properties}
    \centering
...
\begin{frame}{Inference for Copula Models}
    \begin{overprint}
        ...
        \onslide<2>
        \begin{block}{1. Full Joint Likelihood}
					\vspace{0.2cm}
        ...
        \onslide<3>
        \begin{block}{2. Inference Functions for Margins (IFM)}
		\vspace{0.2cm}
