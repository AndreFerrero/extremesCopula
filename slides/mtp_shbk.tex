...
\textbf{Extension to extremes: $D(u_n)$ condition} \cite{leadbetter1974}
...
\begin{frame}{Distribution of Maxima of Stationary Sequences}
    ...
    Assume $\{X_n\} \sim F$ is a stationary sequence satifying $D(u_n)$ with maximum $M_n$. Assume also that for the i.i.d. sequence $\{X^*_n\} \sim F$, the associated maxima $M^*_n$ has GEV limit $G_\xi$. \pause Then, for a constant $0 < \theta \leq 1$ (\alert{extremal index}),

    \[
    P \left(\frac{M_n - b_n}{a_n} \le x \right) \xrightarrow{n \to \infty} G_\xi^\theta(x).
    \] \pause

    \textbf{Note:} Under i.i.d, $\theta = 1$ and therefore we recover the standard result.
\end{frame}
...
\begin{frame}{Extremes of Markov Chains}
	...
	\begin{itemize}
		\item Extremal index of Markov Chains \cite{smith1992}
		\item \textbf{Tail Chains:} The evolution of extremes is governed by a latent process \cite{drees2015}
...
\begin{frame}{Copula Markov Chains}
    ...
    \begin{overprint}
        ...
        \begin{block}{Properties}
            Copula Markov Chains generally satisfy mixing conditions for many copula choices (e.g. Gumbel) \cite{chen2009}
        \end{block}
