...
\begin{frame}{The Hill Estimator}
    A semi-parametric approach for the tail index \alert{$\xi > 0$}, assuming a regularly varying tail: $\bar{F}(x) = x^{-1/\xi} L(x)$. \pause
...
\begin{frame}{Extended GPD: Avoiding Threshold Selection}
    The idea is to transform the GPD distribution by a suitable distortion function $G(u)$ \cite{naveau2016}. \pause
    ...
    \begin{itemize}
        \item \textbf{Tail Consistency:} For any $\kappa > 0$, the tail behavior remains governed by $\xi$. The EGPD and GPD share the \alert{same tail index}. \pause
...
\begin{frame}{Copula Markov Chains}
    ...
    \begin{block}<3->{Transition Kernel}
        ...
        \[ 
        P(X_t \le y \mid X_{t-1} = x) = \frac{\partial}{\partial u} C(u, F(y)) \Big|_{u=F(x)} 
        \]
    \end{block}
...
\begin{frame}
    \begin{figure}
        \centering
        \includegraphics[width=0.8\textwidth]{figures/egpd_gumbel_hist_ts.png}
    \end{figure}
\end{frame}
...
\begin{frame}
    \begin{figure}
        \centering
        \includegraphics[width=0.8\textwidth]{figures/egpd_gumbel_acf_pacf.png}
    \end{figure}
\end{frame}

\begin{frame}
    \centering
    \only<1>{\includegraphics[width=0.65\textwidth]{figures/egpd_gumbel_lag_plot.png}}
    \only<2>{\includegraphics[width=0.65\textwidth]{figures/egpd_gumbel_lag_plot_upper_zoom.png}}
    \only<3>{\includegraphics[width=0.65\textwidth]{figures/lag_upper.png}}
    \only<4>{\includegraphics[width=0.65\textwidth]{figures/egpd_gumbel_lag_plot_lower_zoom.png}}
    \only<5>{\includegraphics[width=0.65\textwidth]{figures/lag_lower.png}}
\end{frame}
