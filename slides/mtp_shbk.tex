...
\begin{frame}{Distribution of Maxima of Stationary Sequences}
    ...
    \[
    P \left(\frac{M_n - b_n}{a_n} \le x \right) \xrightarrow{n \to \infty} G_\xi^\gamma(x).
    \] \pause
...
\begin{frame}{Copula Markov Chains}
    ...
    \begin{overprint}
        ...
        \begin{block}{Properties}
            Copula Markov Chains generally satisfy mixing conditions for many copula choices (e.g. Gumbel) \parencite{chen2009}
        \end{block}
...
\begin{frame}{Inference for Copula Models}
    Estimating parametric copula models is generally done via:
    ...
    \begin{overprint}
        ...
        \begin{block}{1. Inference Functions for Margins (IFM)}
            ...
            \begin{enumerate}
                \item Estimate marginal parameters $\hat{\bm{\phi}}$ assuming i.i.d. observations.
                \item Estimate copula parameters $\hat{\bm{\theta}}$ given the fitted margins $\hat{F} = F_{\hat{\phi}}$ by maximising copula log-likelihood.
            \end{enumerate}
        \end{block}
...
\begin{frame}{Tail Index: Speed of Convergence}
    Evaluated methods:
    \begin{itemize}
...
\begin{frame}{Tail Index: Copula Mispecification}
    \begin{figure}
        \centering
...
\textbf{Pros}
\begin{itemize}
    \item It captures dependence of extremes and nulk while modelling the entire marginal behaviour
